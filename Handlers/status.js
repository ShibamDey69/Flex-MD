    ...
    switch (messageType) {
      ...
      case 'videoMessage':
        ...
        await Neko.sendMessage(Neko.user.id, {
          video: buffer2,
          caption: `*Status:* "${(m.message.videoMessage?.caption)}" from *${name}*` || `*Status:* From ${name}`
        })
        break;
      case 'imageMessage':
        ...
        await Neko.sendMessage(Neko.user.id, {
          image: buffer3,
          caption: `*Status:* "${(m.message.imageMessage?.caption)}" from *${name}*` || `*Status:* From ${name}`
        })
        break;
      case 'extendedTextMessage':
        Neko.sendMessage(Neko.user.id, { text: `*Status:* "${m.message.extendedTextMessage.text}" from *${name}*` })
        break;
      default:
        break;
    }
